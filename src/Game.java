...
//            double y = Math.random() * 800 - 400;
            double angle = Math.random() * Math.PI * 2;
            double rad = Math.sqrt(Math.random()) * 200;
            double  weight = Math.random();
            list.add(new Particle(weight * weight * weight + 1, Math.cos(angle) * rad - 10000, Math.sin(angle) * rad, Math.sin(angle) * rad / 7000 + (Math.random() - 0.5)/ 10, - Math.cos(angle) * rad / 7000 + (Math.random() - 0.5) / 10 + 0.05, 2, Color.DARK_GRAY, 0));
        }
//        placeParticles(100, 2, 1, 500, 300, 0, 0);
...
//        }
    ...
    public void placeParticles(double size, double rad, double mass, double x, double y, double vx, double vy) {
        for(int i = 0; i < size * size * 4; i++) {
            ...
            list.add(new Particle(mass, rad * fx * 2 + (x  - WIDTH / 2) / scale - this.x, rad * fy * 2 + (y - HEIGHT / 2) / scale - this.y, vx, vy, rad, Color.DARK_GRAY, 0));
        }
    }
