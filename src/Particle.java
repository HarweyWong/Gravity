...
//        if(r < dist * (1 + 0.2 / (1 + p.temp + temp))) {
        if(r < dist) {
            ...
            fx -= fMag * nx;
            fy -= fMag * ny;
            temp += Math.abs(d) * 3  + (p.temp - this.temp)  / 200  ;
        }
    }
    ...
    public void render(Graphics2D g2d, double x, double y, double scale) {
        ...
        g2d.fillOval((int)((this.x + x) * scale + Game.WIDTH / 2  - rad * scale),(int)((this.y + y) * scale + Game.HEIGHT / 2 - rad * scale),Math.max((int)(rad * 2 * scale * 1.3 ),2), Math.max((int)(rad * 2 * scale * 1.3  ),2));
    }
}
